...
public class MiniJavaParseControllerGenerated extends DynamicParseController {
	public static final String LANGUAGE = new String("MiniJava");
	...
	protected static synchronized void setDescriptor(Descriptor descriptor) {
		MiniJavaParseControllerGenerated.descriptor = descriptor;
	}

	protected static void createDescriptor() {
		try {
			InputStream descriptorStream = MiniJavaParseControllerGenerated.class.getResourceAsStream(DESCRIPTOR);
			InputStream table = MiniJavaParseControllerGenerated.class.getResourceAsStream(TABLE);
			...
			if(descriptorStream == null)
				throw new BadDescriptorException("Could not load descriptor file from " + DESCRIPTOR
					+ " (not found in plugin: " + getPluginLocation() + ")");
			if(table == null)
			...
			descriptor.setAttachmentProvider(MiniJavaParseControllerGenerated.class);
	...
	private static String getPluginLocation() {
		return MiniJavaParseController.class.getProtectionDomain().getCodeSource().getLocation().getFile();
	}
